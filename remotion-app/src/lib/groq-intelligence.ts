...
import {getCaptionStyleProfile, type CaptionStyleGroupingPolicy} from "./stylebooks/caption-style-profiles";
import {applyVariationBudget} from "./variation-router";
import type {AppEnv, ChunkIntent, ChunkSemanticMeta, NameSpan, TranscribedWord} from "./types";
import type {WordChunk} from "./caption-chunker";
...
const validateCoverage = (
  ...
  chunks: z.infer<typeof groqResponseSchema>["chunks"],
  ...
  for (const chunk of chunks) {
    ...
    if (wordCount < groupingPolicy.hardMinWords || wordCount > groupingPolicy.hardMaxWords) {
      throw new Error(
        `Groq chunk word count out of supported ${groupingPolicy.hardMinWords}-${groupingPolicy.hardMaxWords} range.`
      );
    }
...
const validateNameLock = (nameSpans: NameSpan[], chunks: z.infer<typeof groqResponseSchema>["chunks"]): void => {
...
const validateVariationBudgetHints = (intents: ChunkIntent[]): void => {
  if (intents.length === 0) {
    return;
  }
...
const buildPrompt = (words: TranscribedWord[], groupingPolicy: CaptionStyleGroupingPolicy): string => {
...
export const buildGroqEnhancedChunks = async ({
  words,
  env,
  fetchImpl = fetch
}: {
  ...
  env: AppEnv;
  fetchImpl?: FetchLike;
}): Promise<{chunks: WordChunk[]; emphasisOverrides: Record<number, number[]>}> => {
  ...
  const hintedIntents: ChunkIntent[] = payload.chunks.map((chunk) => chunk.intent ?? "default");
  validateVariationBudgetHints(hintedIntents);

  const emphasisOverrides: Record<number, number[]> = {};
  const chunks = payload.chunks.map((chunk, chunkIndex) => {
    const wordsForChunk = words.slice(chunk.startIndex, chunk.endIndex + 1);
    emphasisOverrides[chunkIndex] = chunk.emphasisWordIndices.filter(
      (index) => index >= 0 && index < wordsForChunk.length
    );
