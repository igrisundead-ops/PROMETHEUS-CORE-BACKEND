import type {GradeProfile, MotionGradeProfileId, MotionTier} from "../types";

export const gradeProfiles: Record<MotionGradeProfileId, GradeProfile> = {
  neutral: {
    id: "neutral",
    label: "Neutral Clean",
    contrast: 1.03,
...
export const resolveGradeProfile = (profileId: MotionGradeProfileId): GradeProfile => {
  return gradeProfiles[profileId] ?? gradeProfiles.neutral;
};

export const getDefaultGradeProfileIdForTier = (tier: MotionTier): MotionGradeProfileId => {
  if (tier === "minimal") {
    return "neutral";
  }
  if (tier === "editorial") {
...
export const buildGradeFilter = (profile: GradeProfile): string => {
