const transition = (preset) => preset;
...
export const resolveTransitionPreset = (presetId) => {
    return transitionPresets.find((preset) => preset.id === presetId) ?? transitionPresets[0];
};
const tierOrder = ["minimal", "editorial", "premium", "hero"];
export const getTransitionPresetsForTier = (tier) => {
...
export const getDefaultTransitionPresetIdForTier = (tier) => {
    if (tier === "minimal") {
        return "minimal-soft-fade";
    }
    if (tier === "editorial") {
...
export const pickTransitionPresetId = ({ tier, seed, preferredPresetId }) => {
    ...
    return pool[hashString(seed) % pool.length]?.id ?? getDefaultTransitionPresetIdForTier(tier);
};
