import type {ZoomTimingFamily} from "../types";

export type ZoomEaseId =
  | "power1.inOut"
  | "power2.inOut"
  | "power2.out"
  | "power3.out"
...
export type ZoomTimingFamilyDefinition = {
  totalRangeMs: readonly [number, number];
  zoomInRatioRange: readonly [number, number];
  holdRatioRange: readonly [number, number];
  peakScaleRange: readonly [number, number];
  leadInMsRange: readonly [number, number];
  easeIn: ZoomEaseId;
  easeOut: ZoomEaseId;
};

export type ZoomEnvelope = {
  startMs: number;
  peakStartMs: number;
  peakEndMs: number;
  endMs: number;
...
export const ZOOM_TIMING_FAMILY_ORDER: ZoomTimingFamily[] = [
  "assertive",
  "bobby",
  "glide",
  "linger",
...
const ZOOM_TIMING_FAMILY_DEFINITIONS: Record<ZoomTimingFamily, ZoomTimingFamilyDefinition> = {
  assertive: {
...
export const getZoomTimingFamilyDefinition = (family: ZoomTimingFamily): ZoomTimingFamilyDefinition => {
  return ZOOM_TIMING_FAMILY_DEFINITIONS[family];
};

const sortFamiliesForSeed = (families: ZoomTimingFamily[], seed: string): ZoomTimingFamily[] => {
  return [...families].sort((a, b) => {
    return (hashString(`${seed}|${a}`) % 1000) - (hashString(`${seed}|${b}`) % 1000);
  });
};

const dedupeFamilies = (families: ZoomTimingFamily[]): ZoomTimingFamily[] => {
  return [...new Set(families)];
};

export const buildZoomTimingFamilyOrder = ({
  seed,
  preferredFamilies = [],
  previousFamily
}: {
  seed: string;
  preferredFamilies?: ZoomTimingFamily[];
...
export const buildZoomEnvelope = ({
  family,
  seed,
  contentDurationMs,
  contentStartMs,
  ...
  const centeredDurationMs = clampNumber(
    contentDurationMs + 900,
    definition.totalRangeMs[0],
    definition.totalRangeMs[1]
  );
  const durationVariance = Math.round((seededUnit(`${seed}|variance`) - 0.5) * 180);
  const totalMs = clampNumber(
    centeredDurationMs + durationVariance,
    definition.totalRangeMs[0],
    definition.totalRangeMs[1]
  );
  const zoomInRatio = seededNumberInRange(
    `${seed}|zoom-in-ratio`,
    definition.zoomInRatioRange[0],
    definition.zoomInRatioRange[1]
  );
  const holdRatio = seededNumberInRange(
    `${seed}|hold-ratio`,
    definition.holdRatioRange[0],
    definition.holdRatioRange[1]
  );
  ...
  const leadInMs = Math.round(
    seededNumberInRange(
      `${seed}|lead-in`,
      definition.leadInMsRange[0],
      definition.leadInMsRange[1]
    )
  );
  ...
  const peakScale = clampNumber(
    seededNumberInRange(
      `${seed}|peak-scale`,
      definition.peakScaleRange[0],
      definition.peakScaleRange[1]
    ) + scaleBoost,
    definition.peakScaleRange[0],
    1.2
  );
