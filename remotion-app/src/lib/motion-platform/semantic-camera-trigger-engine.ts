import type {CaptionChunk, MotionCameraCue, ZoomTimingFamily} from "../types";
import {
  buildZoomEnvelope,
  buildZoomTimingFamilyOrder
} from "./zoom-timing";

type CameraTriggerCategory =
  | "definition-reveal"
  | "direct-address"
  | "identity-intro"
  | "instruction"
...
type CameraTriggerPattern = {
  id: string;
  category: CameraTriggerCategory;
  score: number;
  phrases: string[];
  timingFamilyBias?: ZoomTimingFamily[];
};

type CameraTriggerWindow = {
  startIndex: number;
  endIndex: number;
  startMs: number;
  endMs: number;
...
export type SemanticCameraTriggerCandidate = {
  ...
  text: string;
  patternIds: string[];
  preferredTimingFamilies: ZoomTimingFamily[];
  reason: string;
};
...
const CAMERA_TRIGGER_PATTERNS: CameraTriggerPattern[] = [
  {
    id: "for-you-payoff",
...
const getCameraCueDurationMs = (chunks: CaptionChunk[]): number => {
  if (chunks.length === 0) {
    return 0;
  }
...
export const getTargetCameraCueCount = (chunks: CaptionChunk[]): number => {
...
const buildCameraTriggerWindows = (chunks: CaptionChunk[]): CameraTriggerWindow[] => {
  const windows: CameraTriggerWindow[] = [];
...
const getMatchedPatterns = (window: CameraTriggerWindow): CameraTriggerPattern[] => {
  return CAMERA_TRIGGER_PATTERNS.filter((pattern) => {
    return pattern.phrases.some((phrase) => window.normalizedText.includes(phrase));
  });
};

const resolveWindowAnchorIndex = ({
  chunks,
  window,
  matchedPatterns
}: {
  chunks: CaptionChunk[];
  window: CameraTriggerWindow;
  matchedPatterns: CameraTriggerPattern[];
}): number => {
...
const buildFallbackPatternIds = (chunks: CaptionChunk[], window: CameraTriggerWindow): string[] => {
  const windowChunks = chunks.slice(window.startIndex, window.endIndex + 1);
...
const getPreferredTimingFamilies = ({
  chunks,
  window,
  matchedPatterns
}: {
...
const scoreCameraTriggerWindow = ({
  chunks,
  window
}: {
  chunks: CaptionChunk[];
  window: CameraTriggerWindow;
}): {score: number; patternIds: string[]; reason: string; anchorIndex: number} => {
...
const buildCueForCandidate = ({
  candidate,
  previousFamily
}: {
  candidate: SemanticCameraTriggerCandidate;
  previousFamily?: ZoomTimingFamily;
}): MotionCameraCue => {
...
export const buildSemanticCameraTriggerCandidates = (chunks: CaptionChunk[]): SemanticCameraTriggerCandidate[] => {
  const candidateMap = new Map<string, SemanticCameraTriggerCandidate>();
  ...
  buildCameraTriggerWindows(chunks).forEach((window) => {
    ...
    const candidate: SemanticCameraTriggerCandidate = {
      anchorChunkId,
...
export const selectSemanticCameraCueMap = (chunks: CaptionChunk[]): Map<string, MotionCameraCue> => {
  const selectedCandidates: SemanticCameraTriggerCandidate[] = [];
  ...
  const selected = new Map<string, MotionCameraCue>();
  let previousFamily: ZoomTimingFamily | undefined;
  selectedCandidates
