...
const normalizeWord = (value) => value.replace(/[\u2018\u2019]/g, "'").replace(/[^a-zA-Z0-9']/g, "").toLowerCase();
const isTitleLike = (value) => /^[A-Z][a-z]+(?:'[A-Za-z]+)?$/.test(value);
const isAlphabeticWord = (value) => /^[A-Za-z][A-Za-z'-]*$/.test(value);
const toChunkText = (words) => words.map((word) => word.text).join(" ").trim();
...
export const detectNameSpans = (words) => {
    if (words.length === 0) {
        return [];
    }
    const spans = [];
    for (let index = 0; index < words.length; index += 1) {
        ...
        if (NAME_CUE_WORDS.has(previousNormalized) &&
...
const resolveIntent = (chunkWords, nameSpans) => {
...
const scoreBoundary = ({ start, end, words, nameSpans, chunkingPolicy }) => {
    ...
    const estimateLineLength = (from, to) => {
        if (from > to) {
            return 0;
        }
        let length = 0;
...
export const buildSemanticChunks = (words, chunkingPolicy = captionPolicy.chunking) => {
    if (words.length === 0) {
        return [];
    }
...
export const applyProperCaseToNameWords = (words, nameSpans) => {
    if (nameSpans.length === 0 || !captionPolicy.styling.keepProperCaseNames) {
        return words;
    }
    const nameWordIndexes = new Set();
    nameSpans.forEach((span) => {
