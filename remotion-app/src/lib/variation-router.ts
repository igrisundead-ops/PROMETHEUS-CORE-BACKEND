...
import type {ChunkIntent} from "./types";
import type {SemanticWordChunk} from "./chunk-semantics";

const intentPriority = (intent: ChunkIntent): number => {
  switch (intent) {
    case "name-callout":
      return 2;
    case "punch-emphasis":
...
export const applyVariationBudget = (chunks: SemanticWordChunk[]): SemanticWordChunk[] => {
...
export const getVariationStats = (chunks: SemanticWordChunk[]) => {
  const variationCount = chunks.filter((chunk) => chunk.semantic.isVariation).length;
  const intents = chunks.reduce<Record<ChunkIntent, number>>(
    (acc, chunk) => {
