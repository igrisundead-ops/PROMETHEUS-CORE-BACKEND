const SAFE_THREE_WORD_CARD_VARIANTS = [
    "dream-big-now",
    "your-master-mind",
    "take-action-now",
    "build-legacy-your"
];
const inlineThreeWordStyle = {
    styleKey: "trio_tall_punch_middle",
    motionKey: "three_word_tall_blade",
    layoutVariant: "inline"
};
const threeWordCardStyles = {
    "dream-big-now": {
...
const normalizeWord = (word) => word.replace(/[\u2018\u2019]/g, "'").trim();
const normalizeWords = (words) => words.map(normalizeWord).filter(Boolean);
...
export const canUseThreeWordCard = (words) => {
    ...
    const punctuationChars = cleanWords.reduce((sum, word) => sum + (word.match(/[.,!?;:()[\]{}\-"]/g)?.length ?? 0), 0);
    if (punctuationChars > 2) {
        return false;
    }
    return true;
};
export const pickThreeWordCardVariant = (words) => {
...
export const getThreeWordCardStyle = (variant) => {
    if (variant === "inline") {
        return inlineThreeWordStyle;
    }
    return threeWordCardStyles[variant];
};
export const SAFE_THREE_WORD_CARD_STYLE_KEYS = SAFE_THREE_WORD_CARD_VARIANTS.map((variant) => threeWordCardStyles[variant].styleKey);
