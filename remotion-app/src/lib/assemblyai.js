...
const getUploadHeaders = (apiKey) => ({
    authorization: apiKey,
    "content-type": "application/octet-stream"
});
export const uploadToAssemblyAI = async ({ filePath, apiKey, fetchImpl = fetch }) => {
...
export const createAssemblyTranscriptJob = async ({ uploadUrl, apiKey, fetchImpl = fetch }) => {
    ...
    if (!response.ok) {
        throw new Error(`AssemblyAI transcript create failed (${response.status}): ${await response.text()}`);
    }
    const payload = createTranscriptResponseSchema.parse(await response.json());
    return payload.id;
};
export const pollAssemblyTranscriptJob = async ({ transcriptId, apiKey, fetchImpl = fetch, pollIntervalMs = 2500, maxPollAttempts = 240 }) => {
    for (let attempt = 0; attempt < maxPollAttempts; attempt += 1) {
        ...
        if (!response.ok) {
            throw new Error(`AssemblyAI transcript polling failed (${response.status}): ${await response.text()}`);
        }
...
export const normalizeAssemblyWords = (transcript) => {
...
export const transcribeWithAssemblyAI = async ({ filePath, apiKey, fetchImpl = fetch, pollIntervalMs = 2500, maxPollAttempts = 240 }) => {
