...
import {getCaptionStyleProfile, isSvgCaptionStyleProfile} from "./stylebooks/caption-style-profiles";
...
const toChunk = (words: TranscribedWord[], semantic: ChunkSemanticMeta): WordChunk => {
...
export const deterministicChunkWords = (
  words: TranscribedWord[],
  ...
  return applyVariationBudget(
    semanticChunks.map((chunk) => toChunk(chunk.words, chunk.semantic))
  );
};

export const mapWordChunksToCaptionChunks = (
  chunks: WordChunk[],
  emphasisOverrides?: Record<number, number[]>,
  options: ChunkerOptions = {}
): CaptionChunk[] => {
  const profileId = options.profileId ?? "slcp";
  const svgSelectionState = isSvgCaptionStyleProfile(profileId)
...
export const getChunkPolicyStats = (chunks: WordChunk[], options: ChunkerOptions = {}) => {
  const profile = getCaptionStyleProfile(options.profileId);
  ...
  const softRangeCount = chunks.filter(
    (chunk) => chunk.words.length >= softMinWords && chunk.words.length <= softMaxWords
  ).length;
  const hardRangeViolations = chunks.filter(
    (chunk) => chunk.words.length < hardMinWords || chunk.words.length > hardMaxWords
  ).length;
  ...
  const nameSplitViolations = chunks.reduce((count, chunk) => {
    const hasInvalidNameSpan = chunk.semantic.nameSpans.some(
      (span) => span.startWord < 0 || span.endWord >= chunk.words.length || span.endWord < span.startWord
    );
    return count + (hasInvalidNameSpan ? 1 : 0);
  }, 0);
  const wordCountHistogram = chunks.reduce<Record<string, number>>((hist, chunk) => {
...
export const makeChunkFromGroqIndices = ({
  words,
  startIndex,
  endIndex,
  semantic
