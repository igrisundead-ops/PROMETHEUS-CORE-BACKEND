...
describe("motion platform", () => {
  ...
  it("prefers text-matched minimalist assets when query terms are available", () => {
    const library: MotionAssetManifest[] = [
      {
  ...
  it("creates composition models for every tier without breaking caption linkage", () => {
    ...
    for (const tier of ["minimal", "editorial", "premium", "hero"] as const) {
      ...
      expect(model.showcaseIntelligencePlan.tier).toBe(model.tier);
      expect(model.showcaseIntelligencePlan.reasons.length).toBeGreaterThan(0);
      expect(model.backgroundOverlayPlan.cues.every((cue) => hasValidMediaTrimWindow({
        trimBeforeFrames: cue.trimBeforeFrames,
        trimAfterFrames: cue.trimAfterFrames
      }))).toBe(true);
      expect(model.soundDesignPlan.cues.every((cue) => hasValidMediaTrimWindow({
        trimBeforeFrames: cue.trimBeforeFrames,
        trimAfterFrames: cue.trimAfterFrames
      }))).toBe(true);
      expect(model.soundDesignPlan.musicCues.every((cue) => hasValidMediaTrimWindow({
        trimBeforeFrames: cue.trimBeforeFrames,
        trimAfterFrames: cue.trimAfterFrames
      }))).toBe(true);
    }
  ...
  it("keeps semantic sidecall cues in the showcase plan when the governor chooses templates or typography", () => {
    const chunks = [
      makeChunk({
        id: "sidecall-a",
        ...
        startMs: 0,
        endMs: 2100,
        words: buildWords("Our growth hit 300 percent year over year.", 0, 2100)
      }),
      makeChunk({
        id: "sidecall-filler-1",
        profileId: "longform_semantic_sidecall_v1",
        text: "Stay focused on the story.",
        startMs: 2600,
        endMs: 3900,
        words: buildWords("Stay focused on the story.", 2600, 3900)
      }),
      makeChunk({
        id: "sidecall-filler-2",
        profileId: "longform_semantic_sidecall_v1",
        text: "Let the speaker carry the beat.",
        startMs: 4300,
        endMs: 5600,
        words: buildWords("Let the speaker carry the beat.", 4300, 5600)
      }),
      makeChunk({
        id: "sidecall-b",
        profileId: "longform_semantic_sidecall_v1",
        ...
        startMs: 8400,
        endMs: 10400,
        words: buildWords("I built my eBay marketplace from scratch.", 8400, 10400)
      }),
      makeChunk({
        id: "sidecall-filler-3",
        profileId: "longform_semantic_sidecall_v1",
        text: "Then the next section settles down.",
        startMs: 11200,
        endMs: 12500,
        words: buildWords("Then the next section settles down.", 11200, 12500)
      }),
      makeChunk({
        id: "sidecall-filler-4",
        profileId: "longform_semantic_sidecall_v1",
        text: "Keep the composition clean and readable.",
        startMs: 13000,
        endMs: 14400,
        words: buildWords("Keep the composition clean and readable.", 13000, 14400)
      })
    ];
    ...
    expect(model.showcasePlan.cues.some((cue) => {
      return cue.cueSource === "template-graphic" &&
        cue.templateGraphicCategory === "graph-chart" &&
        cue.asset.virtualAsset === true;
    })).toBe(true);
    expect(model.showcasePlan.cues.some((cue) => {
      return cue.cueSource === "typography-only" && cue.asset.virtualAsset === true;
    })).toBe(true);
  }, 15000);

  it("keeps aggressive full-frame beam overlays out of the auto-selected stack", () => {
    const chunks = [
      makeChunk({id: "chunk-a", text: "Stay grounded.", startMs: 0, endMs: 900}),
      makeChunk({id: "chunk-b", text: "Keep the focus on the speaker.", startMs: 960, endMs: 1900})
    ];
    ...
    expect(model.motionPlan.selectedAssets.some((asset) => asset.id === "premium-lens-beam")).toBe(false);
  });

  it("suppresses ambient overlay assets for long-form talking-head compositions", () => {
    const chunks = [
      makeChunk({id: "chunk-a", text: "Stay grounded.", startMs: 0, endMs: 900}),
      makeChunk({id: "chunk-b", text: "Keep the focus on the speaker.", startMs: 960, endMs: 2100})
    ];
    ...
    expect(model.motionPlan.selectedAssets).toHaveLength(0);
    expect(model.motionPlan.assetFamilies).toHaveLength(0);
    expect(model.scenes.every((scene) => scene.assets.length === 0 && scene.assetIds.length === 0)).toBe(true);
  });

  it("can keep a safer ambient motion-graphics stack enabled for long-form compositions", () => {
    const chunks = [
      makeChunk({id: "chunk-a", text: "Stay grounded.", startMs: 0, endMs: 900}),
      makeChunk({id: "chunk-b", text: "Keep the focus on the speaker.", startMs: 960, endMs: 2100})
    ];
    ...
    const model = buildMotionCompositionModel({
      ...
      tier: "hero",
      fps: 30,
      ambientAssetFamilies: ["frame", "panel", "grid", "light-sweep", "depth-mask"]
    });

    expect(model.motionPlan.selectedAssets.length).toBeGreaterThan(0);
    expect(model.motionPlan.selectedAssets.some((asset) => asset.id === "premium-lens-beam")).toBe(false);
    expect(model.scenes.some((scene) => scene.assets.length > 0)).toBe(true);
  });
  ...
  it("assigns at most two centered camera punch cues with a safe zoom ceiling", () => {
    ...
    cues.forEach((cue) => {
      ...
      expect(["assertive", "bobby", "glide", "linger", "reveal"]).toContain(cue!.timingFamily);
      expect(cue!.endMs - cue!.startMs).toBeGreaterThanOrEqual(1800);
      expect(cue!.endMs - cue!.startMs).toBeLessThanOrEqual(3600);
  ...
  it("fills long-form fallback camera coverage and keeps a slower cue alive near the outro", () => {
    const chunks = Array.from({length: 12}, (_, index) => {
      const startMs = 1000 + index * 15000;
      const endMs = startMs + 1200;
      const text = index % 2 === 0 ? "Stay with this idea." : "Keep listening closely.";
      return makeChunk({
        id: `long-${index}`,
        text,
        startMs,
    ...
    const cues = scenes
      ...
      .filter((cue): cue is NonNullable<(typeof scenes)[number]["cameraCue"]> => Boolean(cue));
    const totalDurationMinutes = (chunks[chunks.length - 1].endMs - chunks[0].startMs) / 60000;
    const minimumExpected = Math.round(Math.max(1, totalDurationMinutes) * 2);
    const lastCue = cues[cues.length - 1];

    expect(cues.length).toBeGreaterThanOrEqual(minimumExpected);
    expect(lastCue).toBeTruthy();
    expect(lastCue!.startMs).toBeGreaterThanOrEqual(chunks[chunks.length - 1].endMs - 28000);
    expect(["bobby", "linger"]).toContain(lastCue!.timingFamily);
  });
});
