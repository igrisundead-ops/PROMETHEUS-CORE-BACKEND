import type {CaptionPolicy} from "./types";

export const captionPolicy: CaptionPolicy = {
  chunking: {
    hardMinWords: 1,
    hardMaxWords: 7,
    softMinWords: 3,
...
export const isHardWordCountAllowed = (count: number): boolean => {
  return count >= captionPolicy.chunking.hardMinWords && count <= captionPolicy.chunking.hardMaxWords;
};

export const isSoftWordCountPreferred = (count: number): boolean => {
