...
import {resolveSchemaMappings} from "../../lib/motion-platform/schema-mapping-resolver";
...
export const scoreAssetForMoment = (asset: CreativeAsset, moment: CreativeMoment): number => {
  ...
  const schemaMatches = resolveSchemaMappings({
    text: `${moment.transcriptText} ${asset.name} ${asset.tags.join(" ")} ${asset.keywords.join(" ")}`
  });
  let score = 0;
  ...
  if (schemaMatches[0]) {
    score += Math.round(schemaMatches[0].semanticConfidence * 10);
  }
