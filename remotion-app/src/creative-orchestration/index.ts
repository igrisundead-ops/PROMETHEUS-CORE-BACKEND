...
export const buildCreativeOrchestrationPlan = async (input: {
  ...
  sourceJobId?: string;
  judgmentInput?: CreativeContext["judgmentInput"];
  ...
  const context: CreativeContext = {
    ...
    availableAssets: input.availableAssets ?? getUnifiedCreativeAssetCatalog(),
  ...
  const orchestratorAdapter = new ExistingAgentOrchestratorAdapter();
  ...
  const allProposals: AgentProposal[] = (
    await Promise.all(
      moments.flatMap((moment) =>
        agents.map(async (agent) => agent.propose(governedContext, moment))
      )
    )
  ).flat();
  ...
  if (criticReview.status === "needs-revision") {
    const revisionContext: CreativeContext = {...governedContext, revisionPass: 1};
    finalDecisionSet = await director.decide(
      revisionContext,
      moments,
      proposalsByMoment,
      1,
  ...
  const debugReport: CreativeOrchestrationDebugReport = {
    ...
    editDecisionPlans: finalDecisionSet.editDecisionPlans ?? [],
