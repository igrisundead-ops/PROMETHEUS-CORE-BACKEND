...
const pickStyle = (moment: CreativeMoment): string => {
  ...
  return moment.energy >= 0.7 ? "blue-depth-glow" : "none";
};
...
export class BackgroundOverlayAgent implements CreativeAgent<CreativeContext> {
  ...
  async propose(context: CreativeContext, moment: CreativeMoment): Promise<AgentProposal[]> {
    ...
    const backgroundStyle = directive?.minimalismLevel === "minimal" || directive?.spatialConstraints.frameNeedsRestraint || directive?.recentSequenceMetrics.preferRestraintNext
      ? "none"
      : pickStyle(moment);
