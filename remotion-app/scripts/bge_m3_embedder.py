...
def main() -> int:
    ...
    try:
        ...
        try:
            ...
            model = SentenceTransformer(args.model_name, trust_remote_code=True, local_files_only=True)
            runtime = "sentence-transformers"
        except Exception as error:
            emit({
                "type": "error",
                "error": (
                    "Neither FlagEmbedding nor sentence-transformers could be loaded for local BGE-M3. "
                    "Install one of them with `python -m pip install FlagEmbedding --no-deps` plus runtime deps, "
                    "or `python -m pip install sentence-transformers`. "
                    f"Details: {error}"
                )
            })
            return 1
