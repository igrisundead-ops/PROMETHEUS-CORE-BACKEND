...
Generated: 2026-05-04T07:54:08.163Z
...
- `hero_serif_primary`: status=doctrine-only; doctrine-benchmark=jugendreisen; runtime-benchmark=none; active=none; doctrine-only=jugendreisen
- `hero_serif_alternate`: status=partial-runtime; doctrine-benchmark=louize; runtime-benchmark=noto-serif-display; active=noto-serif-display; doctrine-only=louize
- `editorial_serif_support`: status=runtime-ready; doctrine-benchmark=fraunces; runtime-benchmark=playfair-display; active=playfair-display, fraunces, crimson-pro, instrument-serif; doctrine-only=none
- `neutral_sans_core`: status=runtime-ready; doctrine-benchmark=dm-sans; runtime-benchmark=dm-sans; active=dm-sans; doctrine-only=none
- `script_accent_rare`: status=doctrine-only; doctrine-benchmark=ivar-script; runtime-benchmark=none; active=none; doctrine-only=ivar-script, bs-acapulko
- `display_sans_pressure_release`: status=doctrine-only; doctrine-benchmark=none; runtime-benchmark=none; active=none; doctrine-only=sokoli, anton, bebas-neue
...
- `Noto Serif Display` -> `DM Sans` (supports, 0.84): When Noto Serif Display carries the monument lane, DM Sans keeps support information precise and stable.
- `Playfair Display` -> `DM Sans` (supports, 0.79): Playfair Display can still ride with DM Sans when the scene wants lighter prestige and clear utility text.
- `Noto Serif Display` -> `Fraunces` (fallback, 0.77): Fraunces gives Noto Serif Display a warmer editorial support partner without competing for the same hero silhouette.
- `Instrument Serif` -> `DM Sans` (supports, 0.76): Instrument Serif remains a hush-luxury support voice when DM Sans handles clarity and pace.
